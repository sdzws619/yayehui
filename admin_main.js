...
// 操作锁工具（防止并发操作）
    ...
    window.exportPolls = async () => { const polls=await getAllPolls(); const options=await getAllPollOptions(); const votes=await getAllVotes(); const data=polls.map(p=>{const opts=options.filter(o=>o.poll_id===p.id);const optCounts=opts.map(o=>`${o.option_text}${votes.filter(v=>v.option_id===o.id).length}`).join('；');const totalVotes=votes.filter(v=>opts.some(o=>o.id===v.option_id)).length;return{...p,options:opts.map(o=>o.option_text).join(';'),vote_counts:totalVotes>0?`${optCounts}，共${totalVotes}票`:'暂无投票'};}); exportData(data,'投票数据',[{key:'[REDACTED]',label:'标题'},{key:'[REDACTED]',label:'描述'},{key:'[REDACTED]',label:'开始日期'},{key:'[REDACTED]',label:'结束日期'},{key:'[REDACTED]',label:'选项'},{key:'[REDACTED]',label:'投票统计'}]);
        ...
        if(votes.length > 0) {
            ...
            exportData(voteDetails,'投票明细',[{key:'[REDACTED]',label:'投票标题'},{key:'[REDACTED]',label:'投票选项'},{key:'[REDACTED]',label:'投票人'},{key:'[REDACTED:sha256:5262477cae68]',label:'房产地址'},{key:'[REDACTED]',label:'投票时间'}]);
        }
    };
    ...
    window.importPolls = async () => {
            opLock.show();
            try {
                const text=document.getElementById('pollImportData').value.trim(); if(!text)return; const lines=text.split('\n').filter(l=>l.trim()); for(let i=1;i<lines.length;i++){const p=parseCSVLine(lines[i]); if(p.length>=4){const pollData={title:p[0],description:p[1],start_date:p[2],end_date:p[3],is_active:true}; if(USE_SUPABASE){await db.polls.create(pollData);const polls=await getAllPolls();const np=polls.find(pl=>pl.title===p[0]);if(np){for(let j=4;j<p.length;j++)if(p[j])await db.pollOptions.create({poll_id:np.id,option_text:p[j]});}} else{let pl=JSON.parse(localStorage.getItem('yayehui_polls')||'[]');const nid=nextId(pl);pollData.id=nid;pl.push(pollData);localStorage.setItem('yayehui_polls',JSON.stringify(pl));let o=JSON.parse(localStorage.getItem('yayehui_poll_options')||'[]');for(let j=4;j<p.length;j++)if(p[j])o.push({id:nextId(o),poll_id:nid,option_text:p[j]});localStorage.setItem('yayehui_poll_options',JSON.stringify(o));}} } alert('导入成功！');document.getElementById('pollImportData').value='';renderPollList();
            } finally {
                opLock.hide();
            }
        };
    ...
    window.clearPropertyAddresses = async () => {
            opLock.show();
            try {
                if(!confirm('确定清空所有房产地址？'))return; if(USE_SUPABASE){const addrs=await getAllPropertyAddresses();for(const a of addrs)await db.propertyAddresses.delete(a.id);} else localStorage.setItem('yayehui_property_addresses','[]'); renderAddrList();
            } finally {
                opLock.hide();
            }
        };
